...
const NotificationsPanel: React.FC<NotificationsPanelProps> = ({ notifications, onMarkAsRead }) => {
    ...
    return (
        <div className="absolute top-full right-0 mt-2 w-80 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md shadow-lg z-20">
            <div className="p-3 border-b border-slate-200 dark:border-slate-700">
                ...
                {unreadCount > 0 && <p className="text-xs text-slate-500 dark:text-slate-400">You have {unreadCount} unread notifications.</p>}
            </div>
            <ul className="max-h-96 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-700">
                {notifications.length > 0 ? notifications.map(n => (
                    <li key={n.id} className={`w-full text-left text-sm hover:bg-slate-50 dark:hover:bg-slate-700 ${!n.is_read ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
                        <button onClick={() => handleNotificationClick(n)} className="w-full text-left p-3">
                    ...
                    <li className="p-4 text-center text-sm text-slate-500 dark:text-slate-400">You have no notifications.</li>
                )}
            </ul>
        </div>
    );
